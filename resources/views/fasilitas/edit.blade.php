            ...
            <div class="p-6 text-gray-900 border-b border-gray-200">
                <form method="POST" action="{{ route('fasilitas.update', $fasilitas->id) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="judul" id="judul" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" value="{{ old('judul', $fasilitas->judul) }}" required>
                        @error('judul')
                    ...
                    <div class="mb-4">
                        ...
                        <p class="text-xs text-gray-500 mb-2">Contoh: fas fa-wifi, fas fa-coffee, fas fa-tv.</p>
                        <div class="flex items-center">
                            <span class="inline-flex items-center justify-center px-3 py-2 border border-r-0 border-gray-300 bg-gray-50 text-gray-500 rounded-l-md w-12 h-[42px]">
                                <i class="{{ $fasilitas->ikon }}" id="icon-preview"></i>
                            </span>
                            <input type="text" name="ikon" id="ikon" class="block w-full rounded-r-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" value="{{ old('ikon', $fasilitas->ikon) }}" required onkeyup="updateIconPreview()">
                        </div>
                        @error('ikon')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('deskripsi', $fasilitas->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
