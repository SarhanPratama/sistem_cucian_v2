        ...
        <div class="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden">
            ...
            <div class="p-4">
                <form action="{{ route('booking.status') }}" method="GET" class="space-y-6 mb-8">
                    <div>
                        <label for="search" class="block text-sm font-medium text-gray-700 mb-1">Nomor Antrean / Plat
                            Nomor</label>
                        <div class="relative">
                ...
                @if (request()->has('search'))
                    <div class="border-t border-gray-200 pt-6">
                        @if ($transaksi)
                            ...
                            @endphp
                            <div class="{{ $statusColor }} rounded-xl p-6 border border-blue-100">
                                <div class="flex items-center justify-between mb-4">
                                    <h3 class="text-lg font-bold text-gray-900">{{ $transaksi->nomor_antrian }}</h3>
                                    <span
                                        class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full {{ $statusColor }} uppercase">
                                        {{ $transaksi->status }}
                                    </span>
                                </div>
                                ...
                                <dl class="space-y-4 text-sm">
                                    <div
                                        class="flex flex-col sm:flex-row sm:justify-between border-b border-gray-100 pb-2 sm:border-0 sm:pb-0">
                                        <dt class="text-gray-500 mb-1 sm:mb-0">Nama</dt>
                                        <dd class="font-medium text-gray-900 sm:text-right">
                                            {{ $transaksi->pelanggan->nama }}</dd>
                                    </div>
                                    <div
                                        class="flex flex-col sm:flex-row sm:justify-between border-b border-gray-100 pb-2 sm:border-0 sm:pb-0">
                                        <dt class="text-gray-500 mb-1 sm:mb-0">Plat Nomor</dt>
                                        <dd class="font-medium text-gray-900 uppercase sm:text-right">
                                            {{ $transaksi->plat_nomor }}</dd>
                                    </div>
                                    <div
                                        class="flex flex-col sm:flex-row sm:justify-between border-b border-gray-100 pb-2 sm:border-0 sm:pb-0">
                                        <dt class="text-gray-500 mb-1 sm:mb-0">Kategori</dt>
                                        <dd class="font-medium text-gray-900 sm:text-right">
                                            {{ $transaksi->layanan->kategori->nama ?? '-' }}</dd>
                                    </div>
                                    <div
                                        class="flex flex-col sm:flex-row sm:justify-between border-b border-gray-100 pb-2 sm:border-0 sm:pb-0">
                                        <dt class="text-gray-500 mb-1 sm:mb-0">Layanan</dt>
                                        <dd class="font-medium text-gray-900 sm:text-right">
                                            {{ ucfirst($transaksi->layanan->nama) }}</dd>
                                    </div>
                                    <div
                                        class="flex flex-col sm:flex-row sm:justify-between border-b border-gray-100 pb-2 sm:border-0 sm:pb-0">
                                    ...
                                    <div
                                        ...
                                        <dd class="font-medium text-gray-900 sm:text-right">
                                            {{ \Carbon\Carbon::parse($transaksi->waktu_pesan)->format('d M Y, H:i') }}</dd>
                                    </div>
                                    @if ($transaksi->catatan)
                                        <div
                                            class="flex flex-col sm:flex-row sm:justify-between border-b border-gray-100 pb-2 sm:border-0 sm:pb-0">
                                            <dt class="text-gray-500 mb-1 sm:mb-0">Catatan</dt>
                                            <dd class="font-medium text-gray-900 sm:text-right">{{ $transaksi->catatan }}
                                            </dd>
                                        </div>
                                    @endif
                                ...
                                @if ($transaksi->status == 'selesai')
                                    <div class="mt-6 text-center">
                                        <p class="text-sm text-green-600 font-medium mb-2"><i
                                                class="fas fa-check-circle mr-1"></i> Kendaraan Anda sudah selesai dicuci!
                                        </p>
                                        <p class="text-xs text-gray-500">Silakan menuju kasir untuk melakukan pembayaran.
                                        </p>
                                    </div>
                                @elseif($transaksi->status == 'menunggu')
                                    <div class="mt-6 text-center">
                                        <p class="text-sm text-yellow-600 font-medium"><i class="fas fa-clock mr-1"></i>
                                            Harap datang sesuai estimasi waktu.</p>
                                    </div>
                                @endif
                            </div>
                        @else
                            <div class="text-center py-8">
                                ...
                                <p class="text-sm text-gray-500">Pastikan Nomor Antrean atau Plat Nomor yang Anda masukkan
                                    benar.</p>
                            </div>
                        @endif
                    </div>
