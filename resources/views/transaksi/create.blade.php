<x-app-layout>
    ...
    @php
        $layananByKategoriUrl = url('/layanan/by-kategori');
    @endphp
    @push('scripts')
    <script>
        (function () {
            ...
            const baseUrl = @json($layananByKategoriUrl);
            ...
            const populateLayanan = (list) => {
                layananSelect.innerHTML = '<option value="">-- Pilih Layanan --</option>';
                list.forEach((layanan) => {
                    const price = new Intl.NumberFormat('id-ID').format(layanan.harga);
                    layananSelect.innerHTML += `<option value="${layanan.id}">${layanan.nama} - Rp ${price}</option>`;
                });
